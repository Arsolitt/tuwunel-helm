...
| `rtc.domain: "https://rtc.ci.example"` (or a port, uppercase or an underscore in the domain) | `- at '/rtc/domain': 'https://rtc.ci.example' does not match pattern '^[a-z0-9]([-a-z0-9]*[a-z0-9])?(\\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*$'` | Write a bare lowercase DNS name: the chart prefixes `https://`/`wss://` itself and writes the value into an Ingress/HTTPRoute host | [Matrix RTC with LiveKit](./rtc.md) |
| `env: {2FA_TOKEN: abc}` (a name starting with a digit) | `- at '': invalid propertyName '2FA_TOKEN'` followed by `  - at '': '2FA_TOKEN' does not match pattern '^[-._a-zA-Z][-._a-zA-Z0-9]*$'` | Rename it - a container env name may not start with a digit and takes only letters, digits, `-`, `_` and `.`; `envRaw`/`extraEnv` report the same rule against `/envRaw/0/name` and `/extraEnv/0/name` | [Configuring the server](./configuration.md) |
| `image.tag: ""` | `- at '/image/tag': minLength: got 0, want 1` | Name a tag: an empty one renders `ghcr.io/matrix-construct/tuwunel:`, which the kubelet refuses as `InvalidImageName` | [Configuring the server](./configuration.md) |
...
| `ingress.enabled: true` with the default `ingress.class: ""` | `If ingress.enabled is set to true, ingress.class is required` | `tuwunnel/ingress.yaml:61:23` | Name the class serving your cluster | [Exposing the homeserver with Ingress](./ingress.md) |
| `gateway.enabled: true` without `gateway.parentRefs` | `gateway.enabled needs gateway.parentRefs: the chart renders HTTPRoutes that attach to a Gateway you run, it does not create one` | `gateway/httproute.yaml:6:4` | Point the route at an existing Gateway | [Exposing the homeserver with Gateway API](./gateway-api.md) |
...
| A value the chart renders as a host carries a scheme | `server_name must be a bare hostname, not a URL: "https://matrix.ci.example"` - the same sentence names the label the value came from, so a scheme under `ingress.extraHosts` reads `ingress.extraHosts must be a bare hostname, not a URL: "https://alias.ci.example"` | the label's own template: `tuwunnel/ingress.yaml:10:23` for `server_name`, `tuwunnel/ingress.yaml:148:15` for `ingress.extraHosts`, `gateway/httproute.yaml:34:73` for `gateway.hostnames`; the delegated domain from `config.global.well_known.server` is refused by its own guard (next row), which fires first | Write a bare hostname. A `:port` is not this failure - Matrix names may carry one and the chart strips it in the host fields while `TUWUNEL_SERVER_NAME`/`config.toml` keep it; a scheme is refused because there is nothing to strip and the host behind it would appear nowhere in the values | [Exposing the homeserver with Ingress](./ingress.md) |
| `config.global.well_known.server` carries a scheme | `config.global.well_known.server must be a bare host:port, not a URL: "https://delegated.ci.example" - it is written into config.toml and served as m.server, and the server refuses a URL at startup` | `tuwunnel/configmap.yaml:18:4` - the ConfigMap renders on every install, and Helm reports the failure through the StatefulSet's `checksum/config` include, `tuwunnel/statefulset.yaml:72:43` | Write a bare `host`/`host:port` - a port is allowed, a scheme never is, and the guard fires with or without an Ingress or Gateway | [Federation and delegation](./federation.md) |
| `rtc.livekit.config.port` is empty (`""` or null) | `rtc.livekit.config.port is empty: the LiveKit container port, this Service, the RTC Ingress and the RTC HTTPRoute are all built from rtc.livekit.config.port, so a release without it cannot be applied; set a port (the chart's default is 7880)` | `rtc/livekit-service.yaml:10:13` | Set a port, or drop your `rtc.livekit.config` override so the `7880` default applies - an empty value renders `port:`/`number:` (null) in all four objects | [Matrix RTC with LiveKit](./rtc.md) |
| `rtc.livekit.config.port` holds a non-numeric value | `rtc.livekit.config.port: "78x0" is not a port number - it has to be digits only, because the same value is rendered as the LiveKit container port, the port of ci-tuwunel-livekit and the backend port of the RTC Ingress and the RTC HTTPRoute` (rendered with the release name `ci`; `ci-tuwunel-livekit` is your `<fullname>-livekit`) | `rtc/livekit-service.yaml:12:4` | Digits only - a media port *range* such as `"7880-7890"` lands here too and belongs under `config.rtc.port_range_start`/`port_range_end` | [Matrix RTC with LiveKit](./rtc.md) |
...
| A live StatefulSet or RTC Deployment still selects its pods with `helm.sh/chart` (a release created by chart 2.0.1 or older) | `the live release still selects its pods with the chart version in spec.selector.matchLabels, which chart 2.0.1 and older rendered from helm.sh/chart. A workload's selector is immutable, so the API server refuses this upgrade ("spec.selector: Invalid value: ...: field is immutable"). Run the commands below once and start the upgrade again.` followed by the ready `kubectl delete` commands for your release | `tuwunnel/statefulset.yaml:3:3`, the `tuwunel.assert-selector-migration` helper | Run the printed commands, then repeat the upgrade — the StatefulSet's pod keeps running; see [Upgrading from 2.0.1 or older](./upgrade.md#upgrading-from-201-or-older) | [Upgrading](./upgrade.md) |
| `includeServerName: false` with `ingress.enabled: true` and neither a delegated domain nor `ingress.extraHosts` | `includeServerName=false leaves the Ingress without a hostname: server_name is its only host unless the delegated domain (config.global.well_known.server) or ingress.extraHosts supplies one` | `tuwunnel/ingress.yaml:28:4` | Set `config.global.well_known.server` or an `ingress.extraHosts` entry so the Ingress keeps a rule to route, or drop the value | [Exposing the homeserver with Ingress](./ingress.md) |
| `includeServerName: false` with `gateway.enabled: true` and neither a delegated domain nor `gateway.hostnames` | `includeServerName=false leaves the HTTPRoute without a hostname: server_name is its only hostname unless the delegated domain (config.global.well_known.server) or gateway.hostnames supplies one; a route with an empty hostnames list matches every hostname its listener serves` | `gateway/httproute.yaml:40:4` | Set `config.global.well_known.server` or a `gateway.hostnames` entry - a route with an empty `hostnames` list would match every hostname the listener serves | [Exposing the homeserver with Gateway API](./gateway-api.md) |
| `serverNameWellKnownOnly: true` with `ingress.enabled: true` and no delegated domain | `serverNameWellKnownOnly=true needs config.global.well_known.server: without a delegated domain the server_name host is the only host this Ingress serves, so narrowing it to /.well-known/matrix would leave every other path unroutable` | `tuwunnel/ingress.yaml:38:4` | Set `config.global.well_known.server` - the apex needs a host to hand the rest of its paths to - or drop the value | [Exposing the homeserver with Ingress](./ingress.md) |
| `serverNameWellKnownOnly: true` with `gateway.enabled: true` and no delegated domain | `serverNameWellKnownOnly=true needs config.global.well_known.server: without a delegated domain the server_name host is the only hostname the route serves, so narrowing it to /.well-known/matrix would leave every other path unroutable` | `gateway/httproute.yaml:50:4` | The same on the Gateway path: the second route the value renders carries only the apex, so the delegated domain is what serves everything else | [Exposing the homeserver with Gateway API](./gateway-api.md) |

The statefulset template is the one that always renders, so this guard is included from there; a
`lookup` with no cluster to ask returns empty, which is why `helm template` and `helm lint` - and CI,
which has no cluster - never see it.

The 17 schema fixtures live in `../charts/tuwunel/ci/invalid/` and the 12 guard fixtures in `../charts/tuwunel/ci/invalid-render/`. Only the guard fixtures carry an `# expect-error:` comment naming the message their render must report - the `schema` job extracts that substring and fails the fixture if the output does not contain it. The schema fixtures carry no such comment: the job only requires the render to be refused with `schema` in the message.
...
Setting `config.global.well_known.server` switches the Ingress into its delegated layout: on the `server_name` host only `/.well-known/matrix*` and `/_matrix*` are routed, and every other path (`/_tuwunel/server_version`, the RTC control paths `/get_token`, `/sfu/get`, `/healthz`, `/delegate_delayed_leave`, `/sfu_webhook`, admin paths) matches no rule. The Gateway path describes the same host its own way - `<fullname>-server-name` carries those two prefixes and never `/`, while the catch-all route drops the apex - so a cutover between the two shows the same path coverage in different shapes. Setting only `well_known.client` changes nothing in the rendered Ingress or HTTPRoute. A delegated domain equal to `server_name` additionally renders a duplicated TLS host entry and two rules for the same host, the first one being the narrowed rule; on Gateway API the two routes share the host instead. With `includeServerName: false` the apex entry is not rendered at all, so that duplication disappears with it. See [Federation and delegation](./federation.md).

| Symptom | Cause | Fix | Owner |
| --- | --- | --- | --- |
| A single-page site on the apex root (`https://example.com/`) does not reach its own service, or the apex answers the homeserver there | With delegation the chart claims only the Matrix prefixes on the apex - `/.well-known/matrix` and `/_matrix`, never `/` - on the Ingress rule and on the Gateway apex route, which used to ride the catch-all route instead, so an upgraded Gateway install loses the root coverage it previously had; the root is unclaimed by design and nothing in the chart routes it | Attach your own object for the root: a second Ingress, or an `HTTPRoute` on the same host and listener. Without delegation the apex is the homeserver and its single rule does claim `/`; on Gateway API, listing the apex in `gateway.hostnames` puts the catch-all route back in front of it | [Exposing the homeserver with Ingress](./ingress.md#serving-the-matrix-hostnames), [Gateway API](./gateway-api.md#the-apex-route) |

### A certificate that never issues

`ingress.tls: true` renders one `spec.tls` entry with one secret, so cert-manager issues a single `Certificate` covering the whole host list: `server_name`, the delegated domain and `ingress.extraHosts`. When the apex is served elsewhere - its DNS points at another host, which is the layout upstream's root-domain guide calls [Option 1: static JSON files](https://matrix-construct.github.io/tuwunel/deploying/root-domain-delegation.html#option-1-static-json-files) - no HTTP-01 challenge for `server_name` can succeed, and one failed authorization fails the order as a whole: the delegated host never gets its certificate either, although its DNS is right. `includeServerName: false` drops the `server_name` entry from the Ingress rules and the TLS host list (and from the HTTPRoute hostnames), leaving the certificate to cover exactly the names that resolve to this release. Do not set it without another host: the render refuses to leave either exposure path without one.

| Symptom | Cause | Fix | Owner |
| --- | --- | --- | --- |
| The certificate for the delegated host never issues while its DNS is right, and the failing `Order`/`Certificate` names `server_name` | One `spec.tls` entry is one certificate over the whole list, and a challenge for a name that resolves elsewhere fails the order as a whole | Set `includeServerName: false` and serve the apex from wherever it points - the static-document layout | [Federation and delegation](./federation.md) |
| `helm template` stops with `includeServerName=false leaves the Ingress without a hostname: …` | `includeServerName: false` with neither a delegated domain nor an `ingress.extraHosts` entry: the Ingress would have no rule to route | Set `config.global.well_known.server` or an `ingress.extraHosts` entry, or drop the value ([the guard table](#rejected-at-render-time-template-guards)) | [Exposing the homeserver with Ingress](./ingress.md) |
| `helm template` stops with `includeServerName=false leaves the HTTPRoute without a hostname: …` | The same on the Gateway path, where an empty `hostnames` list would match every hostname the listener serves instead of none | Set `config.global.well_known.server` or a `gateway.hostnames` entry, or drop the value ([the guard table](#rejected-at-render-time-template-guards)) | [Exposing the homeserver with Gateway API](./gateway-api.md) |
...
| A `well_known.server` written as a URL is refused by the render | The key is written into `config.toml` and served as `m.server`, so it has to be a bare `host`/`host:port` - and the render refuses a URL at that point, regardless of whether Ingress or Gateway API is enabled: `config.global.well_known.server must be a bare host:port, not a URL: "https://delegated.ci.example" - it is written into config.toml and served as m.server, and the server refuses a URL at startup` | Write a bare host - a port is allowed and stripped in the host fields; a scheme is not ([Rejected at render time](#rejected-at-render-time-template-guards)) | [Federation and delegation](./federation.md) |
| A `well_known.client` with a port: the install starts, the published document is non-conformant | The chart does not look at `client` - it is not rendered as a host field - so `client` accepts the value and the server serves it verbatim as `m.homeserver.base_url` | Write a portless HTTPS URL; nothing in the chart or the server refuses the port | [Federation and delegation](./federation.md) |

### Wrong Host

The chart normalises the host values it renders - a trailing `:port` is stripped, a scheme fails the render ([Rejected at render time](#rejected-at-render-time-template-guards)) - but not what they resolve to: routing is per-host (Ingress rules and HTTPRoute hostnames built from `server_name`, the delegated domain and `extraHosts`/`gateway.hostnames`). A request whose host matches no rule is not routed to the homeserver at all. For Gateway API, `gateway.hostnames` entries only take effect if the Gateway listener also serves those hostnames - hostname and TLS binding belong to the listener, which the chart does not create. See [Exposing the homeserver with Gateway API](./gateway-api.md).
...
Route preconditions (`parentRefs`, pod-mode media, a single `udp_port`) are refused at render time - see [Rejected at render time](#rejected-at-render-time-template-guards) for the exact messages. Two RTC values are refused by the layer above, before any RTC object is written: `rtc.domain` by the schema ([Rejected before render](#rejected-before-render-schema)) and `rtc.livekit.config.port` by the LiveKit Service template ([Rejected at render time](#rejected-at-render-time-template-guards)).
...
| Symptom | Cause | Check | Fix | Owner |
| --- | --- | --- | --- | --- |
| `spec.selector: Invalid value: {…,"helm.sh/chart":"tuwunel-2.0.2"}: field is immutable` on the first upgrade of a release created by chart ≤ 2.0.1 | The selector labels of the StatefulSet and the RTC Deployments included `helm.sh/chart`, which moves with every chart release, and a workload's selector is immutable — the upgrade fails at the API server, naming all three objects | `kubectl -n <namespace> get statefulset <fullname> -o jsonpath='{.spec.selector.matchLabels}'` shows the `helm.sh/chart` entry | Delete the workloads once and repeat the upgrade: from 2.0.2 the render prints the exact commands for your release, and they leave the homeserver pod running — [Upgrading from 2.0.1 or older](./upgrade.md#upgrading-from-201-or-older) | [Upgrading](./upgrade.md) |
