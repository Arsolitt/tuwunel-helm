...
| `ingress.enabled: true` with the default `ingress.class: ""` | `If ingress.enabled is set to true, ingress.class is required` | `tuwunnel/ingress.yaml:50:23` | Name the class serving your cluster | [Exposing the homeserver with Ingress](./ingress.md) |
...
| A value the chart renders as a host carries a scheme | `server_name must be a bare hostname, not a URL: "https://matrix.ci.example"` - the same sentence names the label the value came from, so a scheme under `ingress.extraHosts` reads `ingress.extraHosts must be a bare hostname, not a URL: "https://alias.ci.example"` | the label's own template: `tuwunnel/ingress.yaml:9:23` for `server_name`, `tuwunnel/ingress.yaml:131:15` for `ingress.extraHosts`, `gateway/httproute.yaml:21:73` for `gateway.hostnames`; the delegated domain from `config.global.well_known.server` is refused by its own guard (next row), which fires first | Write a bare hostname. A `:port` is not this failure - Matrix names may carry one and the chart strips it in the host fields while `TUWUNEL_SERVER_NAME`/`config.toml` keep it; a scheme is refused because there is nothing to strip and the host behind it would appear nowhere in the values | [Exposing the homeserver with Ingress](./ingress.md) |
