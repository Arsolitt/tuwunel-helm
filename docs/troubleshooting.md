...
### Apply-time rejections

These pass the render and are refused by the API server, which is why they look like cluster problems.

| Message (from `kubectl apply`) | Cause | Fix | Owner |
| --- | --- | --- | --- |
| `spec.tls[0].hosts[1]: Invalid value: ""` | `ingress.tls: true` without `config.global.well_known.server`: the TLS hosts list is built from `server_name` + delegated domain + `extraHosts` with no filtering, and the delegated entry renders empty | Add the delegation block (or drop TLS); verify with `helm upgrade … --dry-run=server` | [Exposing the homeserver with Ingress](./ingress.md) |
| `spec.clusterIPs[0]: Invalid value: "None": may not be set to 'None' for LoadBalancer services` | `service.type: LoadBalancer`/`NodePort` combined with the always-rendered `service.clusterIP: "None"` | Clear the field as well (`--set service.type=LoadBalancer --set service.clusterIP=`) | [Exposing the homeserver with Ingress](./ingress.md) |

Use `--dry-run=server` to catch this class before touching the release; `--hide-secret` if the render contains credentials.
...
| Pod never starts after enabling scheduled backups | `backup.scheduled: true` with `backup.enabled: false` renders the sidecar *and* the `backup-crontabs` volume, but the ConfigMap they reference is guarded by `enabled AND scheduled`. `helm template` succeeds; the volume then references a ConfigMap that is not in the release | The rendered volumes vs the rendered ConfigMap list | Enable the pair together | [Backups and restore](./backups.md) |
| No backups are ever written although the schedule looks armed | The crontab line is `pkill -USR2 -x tuwunel`, an exact `argv[0]` match on the bare name - a wrapper image whose `argv[0]` is a path matches nothing, and crond reports no error. The signal also only reaches the server because the chart sets `shareProcessNamespace` | `<backup.path>/meta/<id>` and the server's `Created database backup…` line | Keep the process name `tuwunel`, or run a backup from the admin room | [Backups and restore](./backups.md) |
| No backup is ever produced even though the schedule is armed | busybox `crond` in the `backup` sidecar runs as the pod's uid 2020 (pod-level `runAsUser`/`runAsNonRoot`), while the crontab file is named `root`, so crond cannot `setuid`/`setgid` to it: at the minute boundary it aborts the job with `crond: can't set groups: Operation not permitted` and nothing runs | The `<backup.path>/meta/` directory stays empty and the server never logs `Created database backup…`; at the rendered log level (`-l 8`) crond itself prints nothing, so `kubectl logs -c backup` looks clean | Trigger backups by hand until the chart fixes the sidecar: `kubectl exec <fullname>-0 -c backup -- pkill -USR2 -x tuwunel`, or `!admin server backup-database` in an admin room | [Backups and restore](./backups.md) |
| `kubectl logs <pod> -c backup` is empty - is the cron dead? | Busybox crond logs through syslog, which is not running in the pod, so the sidecar's logs are empty in normal operation | The server container's `Created database backup…` line and the repository directory | Trust the repository, not the sidecar log | [Backups and restore](./backups.md) |
