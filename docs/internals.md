...
- **Ingress hosts.** When `config.global.well_known.server` is set, the chart strips its `:port` suffix and uses the result as the delegated domain. The `server_name` host is then narrowed to `/.well-known/matrix` and `/_matrix` — to the well-known prefix alone when `serverNameWellKnownOnly: true`, and dropped altogether when `includeServerName: false` — while the delegated host serves `ingress.path`, and `ingress.extraHosts` are appended. The TLS host list is `server_name` + delegated domain + `ingress.extraHosts`, which neither value changes. See [Ingress](./ingress.md) and [Federation and delegation](./federation.md).
- **Gateway hostnames.** Without a delegated domain the HTTPRoute carries `server_name` + `gateway.hostnames` behind one catch-all rule. With one, the chart splits the roles the way the Ingress splits its rules: `<fullname>` carries the delegated domain + `gateway.hostnames` behind the catch-all, and `<fullname>-server-name` carries the apex behind the Matrix path set (`/.well-known/matrix` and `/_matrix`, never `/`). Both lists are deduplicated with `uniq`, and `includeServerName: false` drops the apex route along with the apex hostname. `serverNameWellKnownOnly: true` narrows that path set to `/.well-known/matrix`. See [Gateway API](./gateway-api.md).
